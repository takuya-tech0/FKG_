...
def combine_databases(main_db_path, source_db_paths):
    ...
    # 結合後のデータベースからExcelにエクスポート
    export_db_to_excel(main_conn, "fbsfkg_all.xlsx")
...
def export_db_to_excel(conn, excel_filename):
    ...
    wb = Workbook()
    ws = wb.active
    ws.append(["property_name", "rent", "rent_tax_classification", "station_route", "station_name", "station_near", "floor", "size_in_tsubo", "address", "current_status", "property_id", "property_site", "detail_link", "detail_contact", "image_src", "first_published_date"])
